...
| [`verify-parity.mjs`](./verify-parity.mjs) | `pnpm verify:parity`                      | **gate**  | Two backends have not drifted: duplicated behaviour cores identical, bindings agree on the root element |
...
green. Two backends disagreeing about one contract leaves BOTH conformance suites green, because
each runs against its own copy. That invisibility is the entire justification — a check for something the compiler already
catches is noise.
