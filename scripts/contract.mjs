    ...
    BACKENDS.filter((b) => !selected || b.framework === selected).map((b) => {
      ...
      return [
        b.framework,
        {
          ...
          formConformance: formConformance(contract, b.framework),
        },
      ];
    }),
  );
...
function main() {
  ...
  if (!args.includes('--pretty')) console.log(JSON.stringify(view, null, 2));
    ...
    for (const [framework, info] of Object.entries(view.backends)) {
      ...
      console.log(`Form conformance: ${info.formConformance.status}`);
      if (info.formConformance.detail) console.log(info.formConformance.detail);
