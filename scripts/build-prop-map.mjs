 ...
 *   CANONICAL — packages/contracts/prop-canon.json, hand-maintained. What the vocabulary
...
import { BACKENDS, surfaceFor } from './contract-lib.mjs';
import { REPO_ROOT, listContracts, contractPaths, readJson, byCodePoint } from './contract-lib.mjs';
...
function measure(backend) {
  ...
  for (const name of components) {
    ...
    for (const entry of surfaceFor(contract, backend)) {
 ...
 */
const CARRIES_VALUES = ['axis', 'controlled', 'uncontrolled', 'input', 'model'];
...
function buildJson(canon, m, flags) {
  ...
  return {
    ...
    _doc:
      ...
      'Generated by scripts/build-prop-map.mjs from the CONTRACTS + ' +
...
function buildMd(json, canon) {
  ...
  L.push('> Generated by `scripts/build-prop-map.mjs` — **do not edit by hand.**');
  L.push(
...
function main() {
  ...
  const backends = Object.fromEntries(
    BACKENDS.map(({ framework }) => {
      const m = measure(framework);
      const result = buildJson(canon, m, findFlags(m.props, canon));
      result.surfaces = Object.fromEntries(
        m.components.map((name) => [
          name,
          surfaceFor(readJson(contractPaths(name).contract), framework),
        ]),
      );
      return [framework, result];
    }),
  );
  const stale = Object.keys(canon.dispositions ?? {}).filter((key) => {
    const [component, prop] = key.split('.');
    return !Object.values(backends).some((b) => b.props[prop]?.usedOn.includes(component));
  });
  const json = {
    _schema: '2.0',
    _doc: 'Public surfaces attributed by backend. Derived from contracts and each backend surface reader.',
    backends,
  };
  const md =
    ...
    Object.entries(backends)
      .map(([framework, data]) => {
        const tables = Object.entries(data.surfaces)
          .map(
            ([name, entries]) =>
              ...
              entries
                .map(
                  (e) =>
                    `| ${e.name} | ${e.role} | ${e.type.replace(/\|/g, '\\|')} | ${e.attribute ?? '?'} | ${e.event ?? '?'} |`,
                )
                .join('\n'),
          )
          .join('\n\n');
        return (
          `## ${framework}\n\n` +
          buildMd(data, canon).replace(/^# /gm, '### ').replace(/^## /gm, '### ') +
          '\n\n' +
          tables
        );
      })
      .join('\n\n');
  const files = [
    [OUT_JSON, JSON.stringify(json, null, 2) + '\n'],
    [OUT_MD, md + '\n'],
  ];
  ...
  if (check)
    for (const [path, expected] of files) {
      if (!existsSync(path) || readFileSync(path, 'utf8') !== expected)
        problems.push(`${path} is out of date ? run pnpm prop-map`);
    }
  ...
  console.log(`prop-map${check ? ':check' : ''}: ${BACKENDS.length} backend surfaces.`);
}
main();
